...
function Header() {
    ...
    return (
        <header className={'header w-full flex justify-between h-24 text-white font-bold absolute top-0 left-0 z-50 px-6 max-md:z-[90] ' +
            (navActive ? " max-md:bg-zinc-950 max-md:fixed max-md:h-full" : ''
                // (pathname === "/" ?' bg-transparent':' bg-gradient-to-b from-black/40 to-transparent')
            )
        }>
            <nav className={"w-full max-w-6xl mx-auto flex justify-between items-start max-md:flex-col max-md:gap-4 my-6"}>
                <div className="text-lg max-md:w-full flex justify-between space-x-2">
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.94 }}
                        className="md:hidden" onClick={() => setNavActive(!navActive)}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" className="bi bi-list" viewBox="0 0 16 16">
                            <path fillRule="evenodd" d="M2.5 12a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zm0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5z" />
                        </svg>
                    </motion.button>
                </div>
                <ul className={"flex items-center space-x-8 max-md:w-full max-md:h-full max-md:flex-col max-md:gap-8 max-md:pt-12 " + (navActive ? "" : " max-md:opacity-0 max-md:pointer-events-none")}>
                    ...
                    <li className='navlink'>
                        <Link href={'https://rust.facepunch.com/buy/'} target='_blank'>
                            <motion.button
                                ...
                                whileHover={{ scale: 1.02 }}
                                whileTap={{ scale: 0.99 }}
                                className="navlink px-4 py-2 text-base font-semibold text-white bg-red-600 rounded-lg shadow-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 md:-ml-4"
